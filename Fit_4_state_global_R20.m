...
Pin = [40000 2 0.3] % initial guesses of parameters
% 
...
in = load('');
for i = 1:length(in.fitstats); chi2(i) = in.fitstats{i}.chi2; end
[s Isort] = sort(chi2);
Ifit = Isort(1);
 gridsearch = 0;
%Ifit = 1;
%% Load data
cpmgfiles = {'Gal3_apo_cpmg.cfi'  'Gal3C_lac_s10_1ul_CPMG_600.cfi' 'Gal3C_lac_s10_2ul_CPMG_600.cfi' ...
    'Gal3C_lac_s11_3ul_CPMG_600.cfi' 'Gal3C_lac_s11_6ul_CPMG_600.cfi' 'Gal3C_lac_s11_8ul_CPMG_600.cfi',...
    'Gal3C_lac_s12_11ul_CPMG_600.cfi' 'Gal3_lac_14ul.cfi' };
...
% Total and free ligand konc
totligC = [0 0.02915 0.05758 0.08555 0.16622 0.21745 0.24775 1.1];
...
for i = 1:length(ResToFit) % loop over each res to fit
    for j = 1:length(freeligC) % loop over each concentration
        for k = 1:length(carrier_freq{j}) % Loop over each carrier freq for each res
            ...
            % order in allP
            % kon X Y k12 k21 ptot kd tc lig   ( shift in order 2 3 1 4,
            % R20, carrier freq
            % 1   2 3  4   5   6   7   8  9:en 
            
            % input to r2cpmg_fourstate_ppm_constrain
            %Pin =[kon X Y ppm2 ppm3 R20 k12 k21 ptot L Kd ppm1 ppm4 Tc sfrq] 
